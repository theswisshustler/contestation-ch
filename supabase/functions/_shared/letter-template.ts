...
function hausseLetterHtml(d: DossierContestation, res: ResultatContestation): string {
  ...
  <h2>I. Situation</h2><table class="faits"><tr><td>Logement</td><td>${esc(d.adresseImmeuble)}, ${esc(d.npa)} ${esc(d.commune)}</td></tr><tr><td>Partie bailleresse / régie</td><td>${esc(d.bailleur.nom)}, ${esc(d.bailleur.adresse)}, ${esc(d.bailleur.npa)} ${esc(d.bailleur.ville)}</td></tr><tr><td>Loyer net avant la hausse</td><td>${esc(d.loyerAvantHausse)} CHF par mois</td></tr><tr><td>Nouveau loyer net annoncé</td><td>${esc(d.loyerApresHausse)} CHF par mois</td></tr><tr><td>Date de notification</td><td>${formatDateFr(d.dateNotificationHausse!)}</td></tr>${d.dateEffetHausse ? `<tr><td>Date d'effet annoncée</td><td>${formatDateFr(d.dateEffetHausse)}</td></tr>` : ''}${Array.isArray(d.motifsHausseDetail) && d.motifsHausseDetail.length ? `<tr><td>Motifs indiqués par le bailleur</td><td>${d.motifsHausseDetail.slice(0, 12).map(esc).join('<br>')}</td></tr>` : ''}</table>
